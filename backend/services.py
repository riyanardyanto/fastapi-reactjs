...
oauth2_scheme = _fastapi_security.OAuth2PasswordBearer(tokenUrl="/api/token")
...
async def create_token(user: _models.User):
    ...
    return dict(assertion=token, token_type="Bearer")


async def get_current_user(
    db: _orm.Session = _fastapi.Depends(get_db), token=_fastapi.Depends(oauth2_scheme)
):
    try:
